...
def run_episode(stacked, lidar, action_history, state, model, max_steps):
    """
    ...
    for t in tf.range(max_steps):
        ...
        batched = [tf.expand_dims(x, 0) for x in [stacked, lidar, action_history, state]]
...
@tf.function
def train_step(stacked, lidar, action_history, state, model, optimizer, gamma, max_steps_per_episode):
    """
    initial_state: tf.Tensor, 
    model: tf.keras.Model, 
  ...
  Runs a model training step."""
    ...
    with tf.GradientTape() as tape:
        ...
        action_probs, values, rewards = run_episode(
            stacked, lidar, action_history, state, model, max_steps_per_episode) 
...
class ActorCritic(tf.keras.Model):
    ...
    def call(self, stacked, lidar, actions, state):
        ...
        x = layers.concatenate([stacked_features, lidar_features, action_features, state_features])
