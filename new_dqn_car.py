...
def run_episode(stacked, lidar, action_history, goal, model, max_steps):
    """
    ...
    for t in tf.range(max_steps):
        ...
        batched = [tf.expand_dims(x, 0) for x in [stacked, lidar, action_history, goal]]
        ...
        stacked, lidar, action_history, goal, reward, done = tf_env_step(action)
...
@tf.function
def train_step(stacked, lidar, action_history, goal, model, optimizer, gamma, max_steps_per_episode):
    """
    initial_state: tf.Tensor, 
    model: tf.keras.Model, 
  ...
  Runs a model training step."""
    ...
    with tf.GradientTape() as tape:
        ...
        action_probs, values, rewards = run_episode(
            stacked, lidar, action_history, goal, model, max_steps_per_episode) 
...
class ActorCritic(tf.keras.Model):
    def __init__(self, num_actions):
        ...
        self.conv1 = layers.Conv2D(2, 3, input_shape=(12,12,2), activation="tanh")
        self.conv2 = layers.Conv2D(2, 3, input_shape=(10,10,2), activation="tanh")
        self.pool = layers.MaxPool2D(2)
        self.fc1 = layers.Dense(360, activation = "tanh")
        self.fc2 = layers.Dense(180, activation = "tanh")
        self.fc3 = layers.Dense(32, activation="tanh")
        self.embedding_1 = layers.Embedding(input_dim=(6), output_dim=1, input_length=5)
    ...
    def call(self, stacked, lidar, actions, goal):
        stacked_features = layers.Flatten()(self.pool(self.conv2(self.conv1(stacked))))
        stacked_features = self.fc3(stacked_features)
        lidar_features = self.fc2(self.fc1(lidar))
        action_features = layers.Flatten()(self.embedding_1(actions))
        goal_features = layers.Flatten()(self.embedding_2(goal))
        
        x = layers.concatenate([stacked_features, lidar_features, action_features, goal_features])
...
with tqdm.trange(max_episodes) as t:
  for i in t:
    ...
    if (i == 1) and load:
        model.load_weights("model/model_data.h5")
    if i % 5 ==0:
        model.save_weights("model/model_data.h5")
