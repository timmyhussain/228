...
class Train:
    ...
    def run_episode(self):
        ...
        for t in range(self.max_number_steps):
            ...
            new_belief, new_obstacles, new_goal, reward, done = self.grid.step(belief, action)
    ...
    def _replay(self):
        ...
        for exp in batch:
            ...
            new_beliefs.append(exp[5].flatten())
            new_obstacles.append(exp[6])
        ...
        # print(q_s.shape)
        for ex in range(min(self.Model.batch_size, len(q_s))):
        ...
        # print(len(q_s))
        self.Model.model.fit({"belief": new_beliefs, "obstacles": new_obstacles, "goal": goals}, {"actions": q_s}, verbose=0)


# trainer = Train(Memory, Grid, Model, 50, epsilon=1, gamma=0.99, epochs=1000)
# trainer.run_episode()
# trainer.train()
...
class Test:
    def __init__(self, Memory, Grid, Model, max_number_steps):
        ...
        self.episode_belief_history= []
        self.episode_position_history = []
        self.Model = keras.models.load_model("final_attempt_model_v2/trained_model.h5")
        self.gamma=0.9
        self.max_depth = 2
        self.error_history = []
